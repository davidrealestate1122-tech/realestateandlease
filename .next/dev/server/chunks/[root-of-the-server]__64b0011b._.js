...
var __TURBOPACK__imported__module__$5b$project$5d2f$realestateandlease$2f$node_modules$2f$axios$2f$lib$2f$axios$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/realestateandlease/node_modules/axios/lib/axios.js [app-route] (ecmascript)");
...
async function fetchFromPublicSource() {
    ...
    try {
        ...
        const response = await __TURBOPACK__imported__module__$5b$project$5d2f$realestateandlease$2f$node_modules$2f$axios$2f$lib$2f$axios$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].get("https://www.realtor.com/homes-for-sale/Dallas-TX/", {
            headers: {
        ...
        $('[data-testid="property-card"]').each((index, element)=>{
            try {
                const $card = $(element);
                // Extract address
                const address = $card.find('[data-testid="property-address"]').text().trim() || $card.find("a[href*='/homes/']").first().text().trim() || "Unknown Address";
                // Extract price
                const priceText = $card.find('[data-testid="property-price"]').text().trim() || $card.find("span").filter((i, el)=>$(el).text().includes("$")).first().text().trim();
                const price = parseInt(priceText?.replace(/[^0-9]/g, "") || "0") || 500000 + Math.random() * 300000;
                // Extract beds/baths
                const beds = parseInt($card.find('[data-testid*="bed"]').text().trim() || "3") || 3;
                const baths = parseInt($card.find('[data-testid*="bath"]').text().trim() || "2") || 2;
                // Extract sqft
                const sqftText = $card.find('[data-testid*="sqft"]').text().trim() || $card.find("span").filter((i, el)=>$(el).text().includes("sqft")).first().text().trim();
                ...
                const listingStatus = statusText.includes("SOLD") ? "SOLD" : statusText.includes("PENDING") ? "PENDING" : "ACTIVE";
...
function generateFallbackListings() {
    ...
    return addresses.map((loc, idx)=>({
            ...
            near_commercial: idx === 6 ? true : false,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__64b0011b._.js.map