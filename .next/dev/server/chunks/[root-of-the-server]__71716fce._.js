...
async function getRegionId(city) {
    const url = `https://www.redfin.com/stingray/do/location-autocomplete?location=${encodeURIComponent(city)}&v=2`;
    const res = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$realestateandlease$2f$node_modules$2f$node$2d$fetch$2f$src$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["default"])(url, {
        headers: {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
        }
    });
    const json = await res.json();
    const match = json.payload?.exactMatches?.[0];
    if (!match) throw new Error("Region not found");
    return {
        regionId: match.regionId,
        regionType: match.regionType
    };
}
async function scrapeSoldProperties(cityOrZip) {
    const { regionId, regionType } = await getRegionId(cityOrZip);
    const endpoint = "https://www.redfin.com/stingray/api/gis";
    ...
    const res = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$realestateandlease$2f$node_modules$2f$node$2d$fetch$2f$src$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["default"])(`${endpoint}?${params.toString()}`, {
        headers: {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
        }
    });
    ...
    return homes.map((p)=>({
            ...
            soldPrice: p.lastSoldPrice ?? p.price ?? 0,
            soldDate: p.lastSoldDate ? new Date(p.lastSoldDate) : new Date(),
            squareFeet: p.sqFt ?? 0,
            yearBuilt: p.yearBuilt ?? 0,
            beds: p.beds ?? 0,
            baths: p.baths ?? 0,
            neighborhoodId: regionId.toString(),
            distanceMiles: 0,
            hasGarage: p.garageSpaces > 0,
            isBusyStreet: false,
...
async function POST(req) {
    try {
        ...
        //   subject.zip
        // )
        const soldProperties = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$realestateandlease$2f$lib$2f$scraprers$2f$zillowScrapper$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["scrapeSoldProperties"])(subject.city, subject.state);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__71716fce._.js.map