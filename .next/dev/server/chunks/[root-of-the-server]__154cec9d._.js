...
async function scrapeSoldProperties(city, zip, neighborhoodId = zip) {
    try {
...
function generateMockData(city, zip, neighborhoodId) {
    const properties = [];
    const streets = [
        "Main",
        "Oak",
    ...
    for(let i = 0; i < 25; i++){
        ...
        properties.push({
            ...
            soldPrice: price,
            soldDate: new Date(Date.now() - Math.random() * 180 * 24 * 60 * 60 * 1000),
            squareFeet: Math.floor(Math.random() * 1300) + 1200,
            yearBuilt: Math.floor(Math.random() * 35) + 1980,
            beds,
            baths,
...
async function POST(req) {
    try {
        ...
        const existingDeals = await __TURBOPACK__imported__module__$5b$project$5d2f$realestateandlease$2f$models$2f$Deal$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].find({});
        const existingDeal = existingDeals.find((deal)=>normalizeAddress(deal.subject.address) === normalizedAddress);
        if (existingDeal) {
        ...
        const costBreakdown = (0, __TURBOPACK__imported__module__$5b$project$5d2f$realestateandlease$2f$lib$2f$underwriting$2f$calculateCosts$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["calculateDealCosts"])({
            purchasePrice,
            adjustedArv,
        ...
        const deal = await __TURBOPACK__imported__module__$5b$project$5d2f$realestateandlease$2f$models$2f$Deal$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].create({
            ...
            explainability: {
                arvExplanation: includedComps.length > 0 ? "Base ARV calculated from median price-per-sqft of selected comps, then adjusted for property conditions" : "Fallback ARV estimated from purchase price and market assumptions, then adjusted for property conditions",
                compSelectionExplanation: includedComps.length > 0 ? `${selectedComps.length} comparable sales automatically selected using ZIP, sale recency, square footage, and year built rules` : "No eligible comparable sales found; ARV estimated using fallback pricing logic",
...
//# sourceMappingURL=%5Broot-of-the-server%5D__154cec9d._.js.map