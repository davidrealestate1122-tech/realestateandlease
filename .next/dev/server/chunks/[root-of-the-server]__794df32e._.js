...
async function fetchFromPublicSource() {
    console.log("🤖 Fetching real listings from Realtor.com (Dallas–Fort Worth)");
    let browser = null;
    try {
        ...
        // Intercept API responses to capture property data
        const apiResponses = [];
        page.on("response", async (response)=>{
            const url = response.url();
            // Capture GraphQL and API responses that contain property data
        ...
        await page.goto("https://www.realtor.com/homes-for-sale/Dallas-TX/", {
        ...
        const listings = parseListingsFromRealtor(html);
        if (listings.length > 0) {
            console.log(`✅ Successfully extracted ${listings.length} real listings`);
            return listings;
        }
        console.log("⚠️  No listings found with primary method, trying alternative...");
        return generateFallbackListings();
    } catch (error) {
        console.error(`❌ Scraping error: ${error.message}`);
        console.log("⚠️  Returning generated sample data...");
        return generateFallbackListings();
    } finally{
        if (browser) {
            await browser.close();
...
/**
 * Parse listings from Realtor.com HTML using multiple selector strategies
 */ function parseListingsFromRealtor(html) {
    ...
    const listings = [];
    console.log("🔍 Attempting to parse listings from HTML...");
    // Strategy 1: Look for property cards with comprehensive data
    const selectors = [
        ...
        "[class*='PropertyCard']",
        "[class*='property-card']",
        "div[role='link'][data-testid*='property']"
    ];
        ...
        if (elements.length > 0) {
            elements.each((idx, element)=>{
                try {
                    ...
                    // Extract price
                    const priceStr = extractText($el, [
                        '[data-testid*="price"]',
                        '[data-testid*="property-price"]',
                        "span"
                    ]);
                    const price = extractPrice(priceStr);
                    if (price < 50000) return; // Invalid price
                    // Extract beds
                    const bedsStr = extractText($el, [
                        '[data-testid*="bed"]',
                        '[data-testid*="beds"]'
                    ]);
                    const beds = extractNumber(bedsStr) || 0;
                    // Extract baths
                    const bathsStr = extractText($el, [
                        '[data-testid*="bath"]',
                        '[data-testid*="baths"]'
                    ]);
                    const baths = extractNumber(bathsStr) || 0;
                    // Extract sqft
                    const sqftStr = extractText($el, [
                        '[data-testid*="sqft"]',
                        '[data-testid*="sqfeet"]'
                    ]);
                    const sqft = extractNumber(sqftStr) || 0;
                    // Extract status
                    const statusStr = extractText($el, [
                        '[data-testid*="status"]',
                        '[data-testid*="badge"]'
                    ]);
                    const status = parseStatus(statusStr);
                    // Try to get zip code from address or URL
                    const href = $el.attr("href") || "";
                    const zipMatch = href.match(/\d{5}/) || address.match(/(\d{5})/);
                    const zip = zipMatch ? zipMatch[1] : "75001";
                    // Extract city from address (assuming "city, state" format)
                    const addressParts = address.split(",").map((p)=>p.trim());
                    const city = addressParts[addressParts.length - 2] || "Dallas";
                    if (address && beds > 0 && baths > 0) {
                        listings.push({
                            externalId: `REALTOR_${Date.now()}_${idx}`,
            ...
            if (listings.length > 0) {
                console.log(`  ✓ Found ${listings.length} valid listings with selector: ${selector}`);
                break;
            }
        }
    }
 ...
 */ function extractText($el, selectors) {
    for (const selector of selectors){
        const text = $el.find(selector).text().trim() || $el.filter(selector).text().trim();
        if (text) return text;
    }
    return "";
}
...
/**
 * Fallback: Generate realistic Dallas listings (ONLY when scraping fails completely)
 */ function generateFallbackListings() {
    console.log("📋 Using generated Dallas-Fort Worth listings as fallback\n");
    const addresses = [
        {
            street: "9855 Oak Street",
            city: "McKinney",
            zip: "75069"
        },
        {
            street: "5234 Maple Avenue",
            city: "Plano",
            zip: "75023"
        },
        {
            street: "1876 Elm Drive",
            city: "Dallas",
            zip: "75201"
        },
        {
            street: "7432 Cedar Lane",
            city: "Arlington",
            zip: "76010"
        },
        {
            street: "3021 Pine Road",
            city: "Irving",
            zip: "75060"
        },
        {
            street: "2145 Birch Court",
            city: "Frisco",
            zip: "75034"
        },
        {
            street: "8765 Walnut Street",
            city: "Carrollton",
            zip: "75006"
        },
        {
            street: "4021 Main Road",
            city: "Irving",
            zip: "75060"
        },
        {
            street: "1145 Birch Lane",
            city: "Frisco",
            zip: "75034"
        },
        {
            street: "465 Eve Street",
            city: "Carrollton",
            zip: "75000"
        }
    ];
    ...
    return addresses.map((loc, idx)=>({
            ...
            sqft: idx >= 5 ? Math.round(2397 + Math.floor(Math.random() * 10 - 5)) : Math.round(2000 + Math.random() * 2000),
            ...
            soldDate: idx >= 5 ? new Date(Date.now() - (90 + Math.random() * 90) * 24 * 60 * 60 * 1000).toISOString().split("T")[0] : new Date(Date.now() - Math.random() * 15552000000).toISOString().split("T")[0],
            yearBuilt: idx >= 5 ? 2005 + Math.floor(Math.random() * 10 - 5) : 2000 + Math.floor(Math.random() * 30),
            updatedAt: new Date().toISOString()
        }));
}
__turbopack_async_result__();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__794df32e._.js.map