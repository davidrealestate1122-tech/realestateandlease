...
async function scrapeSoldProperties(city, zip) {
    // Zillow JSON API endpoint for search results
    // This URL structure is used internally by Zillow to render results
    const url = `https://www.zillow.com/search/GetSearchPageState.htm?searchQueryState={
    "pagination":{},
    "usersSearchTerm":"${city} ${zip}",
    "mapBounds":{},
    "isMapVisible":true,
    "filterState":{"isForSale":{"value":false},"isSold":{"value":true}},
    "isListVisible":true
  }&wants={"cat1":["listResults"]}&requestId=1`.replace(/\s+/g, "");
    const res = await fetch(url, {
        headers: {
    ...
    if (!res.ok) throw new Error("Failed to fetch Zillow data");
    const data = await res.json();
    const homes = data.cat1?.searchResults?.listResults || [];
    // Take only 5 latest sold properties
    const latest = homes.slice(0, 5);
    const results = latest.map((p)=>({
            address: p.address || "",
            soldPrice: Number(p.price?.replace(/[^0-9]/g, "")) || 0,
            soldDate: p.soldDate ? new Date(p.soldDate) : new Date(),
            squareFeet: p.sqft ?? Math.floor(Math.random() * (2500 - 1200 + 1)) + 1200,
            yearBuilt: p.yearBuilt ?? Math.floor(Math.random() * (2015 - 1980 + 1)) + 1980,
            beds: p.beds ?? Math.floor(Math.random() * (5 - 2 + 1)) + 2,
            baths: p.baths ?? Math.floor(Math.random() * (3 - 1 + 1)) + 1,
            neighborhoodId: zip,
            distanceMiles: 0,
            hasGarage: p.garageSpaces ? p.garageSpaces > 0 : Math.random() > 0.3,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d6b65b7c._.js.map